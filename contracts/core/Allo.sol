...
contract Allo is Transfer, Initializable, Ownable, AccessControl {
    ...
    /// @notice Reward for catching fee skirting
    /// @dev 1e18 = 100%
    ...
    /// @param _identityId The identityId of the pool creator in the registry
    /// @param _strategy The address of the strategy
    /// @param _initStrategyData The address of the strategy
    /// @param _token The address of the token that the pool is denominated in
    /// @param _amount The amount of the token to be deposited into the pool
    /// @param _metadata The metadata of the pool
    /// @param _managers The managers of the pool
    function _createPool(
        ...
        Pool memory pool = Pool({
            ...
            strategy: _strategy,
            token: _token,
            amount: 0, // value is set in _fundPool
    ...
    /// @notice passes _data through to the pool's strategy
    /// @notice returns the recipientId from the strategy
    /// @param _poolId id of the pool
    /// @param _data encoded data unique to the pool's strategy
    ...
    /// @notice passes _data & msg.sender through to the pool's strategy
    /// @param _poolId id of the pool
    /// @param _data encoded data unique to the pool's strategy
    ...
    /// @param _datas encoded data unique to the pool's strategy
    ...
    /// @notice Deduct the fee and transfers the amount to the strategy
    /// @param _token The address of the token to transfer
